...
public class ZenLootTableWrapper 
{
	...
	//Create a temporary pool if the pool isn't found for whatever reason
	if(pool == null)
	{
	...
	public void applyTweak(LootTable table, ZenLootTableWrapper wrapper)
	{
	    ...
	    if(pool == null) 
	    {
		MineTweakerAPI.logError(String.format("No loot pool with name %s exists in table %s!", poolName, wrapper.name));
		return;
	    }
